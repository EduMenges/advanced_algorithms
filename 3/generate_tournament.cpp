...
int main(int argc, char* argv[]) {
    ...
    double beta = (argc > 4) ? atof(argv[4]) : 0.1;

    if (n <= 0 || c <= 0 || alpha < 0.0 || alpha > 1.0 || beta < -1.0 || beta > 1.0) {
    ...
    for (int i = 0; i < n; ++i)
        for (int j = i + 1; j < n; ++j)
            p[i][j] = min(1.0, dist(gen) + (i == 0 ? beta : 0.0));

    int N = c * (n * (n - 1)) / 2;
    int samples = static_cast<int>(ceil(alpha * N));
